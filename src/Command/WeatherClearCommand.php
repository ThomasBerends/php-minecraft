...
class WeatherClearCommand extends Command
{
    /**
    ...
    protected static $defaultName = 'weather:clear';
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->defaultStorage->update('weather_clear.nbt', gzencode($nbtService->writeString($structure)));

        $paletteChildren[0]->getChildren()[0]->setValue('minecraft:black_concrete');

        $this->defaultStorage->update('weather_rain.nbt', gzencode($nbtService->writeString($structure)));

        $io->success('Cleared the weather');
